...
def attach_bones_and_animation(donor_gltf, recipient_gltf, root_bone_list = [], targets = ['translation', 'rotation', 'scale'], fix_start = True, fix_length = True):
    ...
    if len(recipient_gltf.animations) > 0 and len(donor_gltf.animations) > 0:
        ...
        don_start = min([x for y in recipient_gltf.animations[0].samplers for x in donor_gltf.accessors[y.input].min])
        don_end = max([x for y in recipient_gltf.animations[0].samplers for x in donor_gltf.accessors[y.input].max])
